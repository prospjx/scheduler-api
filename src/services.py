...
from src.clients import analyze_schedule_with_ai, get_courses_data, get_student_profile
...
async def create_optimized_schedule(
    db: Session, request: GenerateScheduleRequest
) -> dict[str, Any]:
    ...
    if not courses_data:
        raise HTTPException(
            status_code=400,
            detail="Could not fetch any of the requested courses from Course API.",
        )
    ...
    for course in courses_data:
        ...
        for ts in timeslots:
            ...
            for existing_class in weekly_schedule[day]:
                if _times_overlap(
                    start, end, existing_class["start_time"], existing_class["end_time"]
                ):
                    raise HTTPException(
                        status_code=409,
